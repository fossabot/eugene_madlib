...
row_array1 - state variable represented all the information necessary to determine number of rows that need to be generated
value2 - timestamp for the current input value
value3 - value at the time value2
value4 - interval of time in seconds at which rows are to be produces

structure of the row_array1 is as follows:

[0] - last time at for which row will be outputed after the finalfunc call following this sfunc
[1] - last value at for which row will be outputed after the finalfunc call following this sfunc
[2] - previous row time
[3] - previous row value
[4] - current row time
[5] - current row value
[6] - interval to separate rows
[7] - number of rows to be generated by finalfunc
[8] - time interval that elapsed since the last output
*/
...
value3 - value at the time value2
value4 - interval of time in seconds at which rows are to be produces
value5 - time at which output rows should start
